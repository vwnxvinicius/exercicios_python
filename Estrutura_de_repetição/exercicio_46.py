...
while True:
    ...
    if atleta == '':
        ...
        # Loop para coletar os saltos
        for i in range(1, 6):
            salto = float(input(f"Insira a distância do salto: {i} de 5: "))
            saltos.append(salto)
        ...
        print(f"\nMelhor salto: {saltos[-1]}")
        print(f"Pior salto: {saltos[0]}")

        # Loop para conseguir a média dos saltos
        saltos.remove(saltos[-1])
        saltos.remove(saltos[0])
        for i in range(len(saltos)):
            soma_saltos += saltos[i]
        media = soma_saltos/len(saltos)
        print(f"Média dos outros saltos: {'%.1f'}" %media)
        print("\nResultado final:")
